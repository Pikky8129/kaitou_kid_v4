...
#        self._logger.info("Text embeddings started.")
        ...
        # 分割したテキストのトークン数を取得する
        sentence_token_counts = list(
            map(lambda x: len(tokenizer.encode(x)), sentences))

        # テキストを分散表現に変換する
        embeddings = self._text_to_embeddings(
            embedding_model, sentences, sentence_token_counts)
...
#        self._logger.info("Text embeddings finished.")
    ...
    def _text_to_embeddings(
        ...
        sentence_groups = self._group_sentences_by_token_count(
            sentences, sentence_token_counts, 8000)

        embeddings = []
        for i, sentence_group in enumerate(sentence_groups):
#            self._logger.info(
#                f"Now embedding...({i+1}/{len(sentence_groups)})")
            ...
            embeddings.extend([x["embedding"]
                              for x in embedding_response["data"]])
    ...
    def _save_text_embeddings_to_csv(
        ...
        df_text = pd.DataFrame(
            {"sentence": sentences, "embeddings": embeddings,

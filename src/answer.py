...
#        self._logger.info("Answering question started.")
        ...
        # テキストと分散表現を読み込む
        df_text = self._load_text_embeddings_from_csv(
            text_embeddings_file_path)
...
#        self._logger.info("Answering question finished.")
    ...
    def _load_text_embeddings_from_csv(self, text_embeddings_file_path: str) -> pd.DataFrame:
        ...
        df_text["embeddings"] = df_text["embeddings"].apply(
            eval).apply(np.array)
        return df_text
    ...
    def _create_context(
        ...
        for sentence_id in df_text_sorted.index:
            ...
            right_boundary = sentence_id + \
                3 if sentence_id < df_text_sorted.index.max() else sentence_id
            # 20230714_MOD_FOR_KeyError_START
...
#                     candidate_sentence_ids.append(neighbor_id)
            for neighbor_id in range(left_boundary, right_boundary + 1):
                if neighbor_id not in candidate_sentence_ids and neighbor_id in df_text_sorted.index:
                    candidate_sentence_ids.append(neighbor_id)
            # 20230714_MOD_FOR_KeyError_END
        # コンテキストの最大トークン数を超えないように文を絞り込む
        selected_sentence_ids = []
        total_tokens = 0
        for sentence_id in candidate_sentence_ids:
            sentence_tokens = df_text_sorted.loc[sentence_id,
        ...
        for context_fragment in context_fragments:
            ...
            for sentence_id in context_fragment:
                text_fragments.append(
                    df_text_sorted.loc[sentence_id, "sentence"])
...
#            self._logger.error(f"OpenAI ChatGPT API - Error: {e}")
            print(f"OpenAI ChatGPT API - Error: {e}")
            raise e
...
#        print(f"Answer: {completion_response['choices'][0]['text']}")
